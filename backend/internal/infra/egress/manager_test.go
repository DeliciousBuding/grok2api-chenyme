...
func TestBuildOAuthBadRequestDoesNotPoisonEgressNode(t *testing.T) {
	cipher, err := security.NewCipher("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=")
	if err != nil {
		t.Fatal(err)
	}
	...
	if repository.updates != 0 || repository.node.Health != 1 || repository.node.CooldownUntil != nil || repository.node.LastError != "" {
		t.Fatalf("build OAuth 400 poisoned node: updates=%d node=%#v", repository.updates, repository.node)
	}
	if !managerHasClientForNode(manager, 1) {
		t.Fatal("build client was invalidated by an account-specific OAuth 400")
	}
}
